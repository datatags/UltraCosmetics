...
/**
 * AUTHOR: WesJD (Thanks!) https://github.com/WesJD/AnvilGUI
 *
...
public class AnvilWrapper implements IAnvilWrapper {
    ...
    public void sendPacketOpenWindow(Player player, int containerId, String guiTitle) {
        toNMS(player).connection.send(new ClientboundOpenScreenPacket(containerId, MenuType.ANVIL, CustomEntities.toComponent(guiTitle)));
    }
    ...
    private class AnvilContainer extends AnvilMenu {
        public AnvilContainer(Player player, String guiTitle) {
            super(AnvilWrapper.this.getRealNextContainerId(player), ((CraftPlayer) player).getHandle().getInventory(), ContainerLevelAccess.create(((CraftWorld) player.getWorld()).getHandle(), new BlockPos(0, 0, 0)));
            this.checkReachable = false;
            setTitle(CustomEntities.toComponent(guiTitle));
        }
        ...
        @Override
        protected void clearContainer(net.minecraft.world.entity.player.Player entityhuman, net.minecraft.world.Container iinventory) {
        }
